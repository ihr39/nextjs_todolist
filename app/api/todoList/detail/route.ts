...
export async function PUT(req: NextRequest){
    ...
    try{
        ...
        let result = await db.collection('todo_detail').updateOne(
            {_id: new ObjectId(json.id), userid: session.user.userid, date: json.date},
            {'$set': editData}
        )
...
export async function DELETE(req: NextRequest){
    ...
    if(session == null ) return NextResponse.json({errMsg: '로그인 먼저 하세요.'},{status:400})
    try{
        let result = await db.collection('todo_detail').deleteOne({_id: new ObjectId(id), userid: session.user.userid})
