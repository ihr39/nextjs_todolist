...
export default function ModalDiary({update, modalOpen, diarys, onClose, onSave, onDelete}:
    {update: boolean, modalOpen:boolean, diarys:Diary, onClose:()=>void, onSave: (content:Diary)=>void, onDelete: (id:string)=>void}){
    ...
    let deleteDiary = () => {
        fetch('/api/diary?date='+diarys.date,{method:'DELETE'})
        .then((r)=>r.json())
        .then((r)=>{
            if(r.errMsg){
                alert(r.errMsg)
            ...
            <div className="fixed inset-0 flex justify-center items-center">
                ...
                <div className="bg-white p-6 rounded-lg w-xl relative">
                        ...
                        <div className="flex justify-between py-4 px-0">
                            <label htmlFor="message" className="flex block mb-2 text-xl font-medium text-gray-900 dark:text-white">
                                {dateKoreaChange(diarys.date)} 일기
                                {
                                    typeof diarys == 'undefined' ? null :  <Trash2Icon size={28} className="ml-1 transition-colors trashBtn" onClick={deleteDiary}/>
                                }
                            </label>
