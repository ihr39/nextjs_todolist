...
export default function DiaryClient({contents}:{contents: {date:string, content:string}[]}){
    ...
    let handleDeleteDiary = (date: string) => {
        setContentList(list => list.filter((item)=> item.date != date))
    }
    ...
    return(
        <div className="">
            ...
            <ModalDiary update={update} modalOpen={modalOpen} diarys={diarys} 
                onClose={()=>setModalOpen(false)} onSave={handlerSaveDiary} onDelete={handleDeleteDiary}
            />
        </div>
    )
}
